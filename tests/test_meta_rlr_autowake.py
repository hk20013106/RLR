...
def test_autowake_uses_bounded_composite_runner_with_outer_timeout(tmp_path, monkeypatch):
    project = tmp_path / "project"
    project.mkdir()
    config = _config(tmp_path)
    monkeypatch.setenv(autowake.AUTOWAKE_CONFIG_ENV, str(config))
    monkeypatch.setattr(autowake, "_current_revision", lambda _repo, _runner: BASE_SHA)
    monkeypatch.setattr(autowake, "_resolve_verified_worktree", lambda **_kwargs: tmp_path / "verified")
    calls = []
    ...
    monkeypatch.setattr(autowake, "run_bounded_process", fake_runner)
    result = autowake.maybe_wake_meta_rlr(
        project_dir=project,
        task_id="dr-test",
        handler_args=_handler_args(tmp_path),
    ...
    assert result is not None
    assert calls[0]["timeout"] == autowake.AUTOWAKE_OUTER_TIMEOUT
    assert calls[0]["cwd"] == autowake._repo_root()
    assert calls[0]["env"][autowake.AUTOWAKE_RETRY_GUARD_ENV] == "1"


def test_autowake_composite_timeout_fails_closed(tmp_path, monkeypatch):
    project = tmp_path / "project"
    project.mkdir()
    config = _config(tmp_path)
    monkeypatch.setenv(autowake.AUTOWAKE_CONFIG_ENV, str(config))
...
def test_outer_timeout_is_strictly_greater_than_known_inner_budgets():
    known_inner = (
        autowake.DEFAULT_REPAIR_JOB_TIMEOUT
        + autowake.VERIFICATION_COMMAND_TIMEOUT
    )

    assert autowake.AUTOWAKE_OUTER_TIMEOUT == known_inner + autowake.OUTER_SAFETY_MARGIN
    assert autowake.AUTOWAKE_OUTER_TIMEOUT > known_inner
