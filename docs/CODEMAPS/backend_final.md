<!-- Generated: 2026-07-08 | Project: Research Loop Room v0.6 | Authoritative merged codemap -->
<!-- Sources: backend_hermes.md (module/CLI/gate/schema reference) + backend_cc.md (coupling analysis)
     + verification_rerun.md + comparison_rerun_2026-07-08.md (real wc -l / grep -n re-verification)
     Supersedes backend_combine.md (this file = backend_combine.md + 4 newly-quantified utility
     scripts + full 29-command CLI list + research_loop_v03.py LOC correction: 2433, not ~2600) -->

# Backend Architecture - Research Loop Room (RLR) v0.6 (Final)

## Project Type

**Multi-Agent Orchestration Engine** - DAG-based research automation with v0.6 divergence-contract
hardening, isolated cognitive agents, and controlled execution workspace.
...
## Core Modules (all LOC verified `wc -l`, this session)
...
| File | LOC (verified) | Purpose | Status |
|------|------|---------|--------|
| `rebuild_deltas.py` | 342 | Delta recovery | git-untracked (working-tree scratch) |
| `deep_extract_deltas.py` | 110 | Post-hoc delta mining (read-only, no loop-runtime effect) | git-untracked |
| `extract_deltas.py` | 81 | Delta extraction utility | git-untracked; not in any prior codemap |
| `extract_deltas_v2.py` | 71 | Delta extraction utility v2 | git-untracked; not in any prior codemap |
...
| `record-pitfall` / `list-pitfalls` / `pitfall-scan` / `pitfall-status` / `promote-pitfall` | Pitfall ledger ops (full lifecycle: record -> scan -> status -> promote) |
| `route` / `note` / `decision` | Node routing, annotation, decision emit |
| `list` / `show` | Candidate/project listing and inspection |
| `pre-research` / `audit-pre-research` | Pre-research artifact gate (L1/L4 fail-closed rc=3) |
| `demo` | Demo/scaffold mode |

---

## v0.6 Gate Functions (line numbers verified `grep -n "^def _audit_"`, unchanged since 2026-07-07)
...
`build_parser()` at research_loop_v04.py:4531, `main()` at :4822 (verified via tokensave_context,
cross-checked against 4843 total LOC).
...
```
Accessed by: all 15 agents (emit) + L10c (aggregate). Contract Risk: **HIGH** - breaking schema
blocks entire loop. Mitigation: `delta_version` field + compat layer.
...
```
Accessed by: `research_loop_v04.py` (auto-record), L8 Curie (confirm). Mutation: Append-only;
status draft->confirmed. Risk: Low.
...
## DAG Topology (15 nodes, verified by direct read of `DAG_NODES` list literal)
...
10 unique personas (verified): Linnaeus, Einstein, Feynman, Oppenheimer, Fisher, Tukey, Turing,
Curie, Darwin, Jobs. L9a/L9b parallel, mutually invisible. Only L10c sees all deltas.
...
- **pytest 9.1** (25 tests in tests/test_v06_divergence.py, verified via grep count)
...
- **Optional:** `caveman-lite` (runtime digest compressor), `everos-mem` (cross-session memory)
...
**Path B (Cognitive):** Context isolation via DAG-allowed deltas only. No filesystem, no subprocess.
Agent sees only what assemble-context provides.

**Path A (Turing):** Controlled temp directory, command allowlist, resource bounds. Results
collected as artifacts.

**Information invisibility:** L2 doesn't see L1 until committed. L9a/L9b mutually blind. Only
L10c sees everything.

---

## Testing Gaps

| Layer | Coverage | Tests (LOC verified this session) |
|-------|----------|--------|
| DAG topology | Yes | test_v05_gate.py (108) |
| Delta emit | Yes | test_template_contract.py (193) |
| Status transitions | Yes | test_run_loop_guards.py (176) |
| Pre-research gate | Yes | test_v05_gate.py |
| v0.6 gates (divergence, memory, traceability) | Yes | tests/test_v06_divergence.py (473 LOC, 25 tests) |
| Provider dispatch | **No** | MISSING |
| Context assembly | Partial | NEEDS EXPANSION |
| Obsidian sync | **No** | MISSING (test_obsidian_sync.py exists at 116 LOC but per backend_combine.md gap note, coverage incomplete — not independently re-verified this session, flagged as carried over) |
...
Responsibilities: DAG state machine, context assembly, delta validation, status transitions,
Obsidian sync, pitfall ledger integration, pre-research gates, v0.6 gates (9 `_audit_*` functions
total, incl. `_audit_dir` helper), CLI dispatch (29 subcommands, verified), loop memory, ARS
adapter integration.

Risk: Single point of failure for state machine. Gate functions span lines 888-4267
(3380-line range, no registry).
...
Current: subprocess CLI (`research_loop_v04.py assemble-context PROJECT CAND --node L5`).
Risk: shell escaping, encoding mismatches.
...
4 delta-extraction/recovery scripts (`rebuild_deltas.py`, `deep_extract_deltas.py`,
`extract_deltas.py`, `extract_deltas_v2.py`, 604 combined lines) are git-untracked working-tree
files. If still in active use, commit them; if superseded by newer versions of each other
(`extract_deltas.py` vs `extract_deltas_v2.py` naming suggests iteration), consolidate and
...
| **v0.7** | Move/delete legacy v0.3 + v0.5b (2992 lines) | 0.5 day | Remove dead code |
| **v0.7** | Commit or consolidate 4 untracked extract/rebuild-deltas scripts | 0.5 day | Close git-tracking gap |
...
Every LOC and line-number figure in this document is one of:
- **Verified this session** via real `wc -l` / `grep -n` / `grep -oP` / `grep -c`, or direct
  `Read` of the `DAG_NODES` list literal (see `comparison_rerun_2026-07-08.md` for full commands
  and raw output).
- **Verified prior session** (`verification_rerun.md`, 2026-07-08) and re-confirmed identical
  this session (research_loop_v04.py LOC, run_loop.py LOC, gate line numbers).
- Explicitly marked **not independently re-verified this session** where carried over from
  `backend_combine.md` without a fresh check (Obsidian sync test coverage note above).

No number in this document is an estimate. `research_loop_v03.py`'s figure was corrected from
an unverified `~2600` (Hermes estimate, carried into backend_combine.md) to the real `2433`.
...
- `backend_combine.md` (2026-07-07/08) - prior merge of the above two, verified-LOC pass
- `verification_rerun.md` (2026-07-08) - direct wc-l/grep-n ground truth for core 6 modules + gates
- `comparison_rerun_2026-07-08.md` (this session) - re-verification + 4 new utility-script LOC +
  full 29-command CLI list + research_loop_v03.py correction (2433, not ~2600)

**Supersedes `backend_combine.md`.** Retain `backend_combine.md` for history; treat this file
(`backend_final.md`) as the authoritative current reference.
