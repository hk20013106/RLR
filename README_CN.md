...
- **新增 L8.5 文献验证节点**：Curie 的第二实例。L7/L8 出结果后，L8.5 基于**实际结果**检索 PubMed/EuropePMC，验证结论是否与已发表文献一致。同时引入可增长的文献数据库（`manage_literature_db.py`），跨轮复用。
...
### v0.2 — 废弃（保留，仍可运行）
- **做了什么**：重构为**门控的 10 人格委员会**（Linnaeus … Jobs），单一共享上下文，带决策日志和 Obsidian 同步。
- **修复了 v0.1 的问题**：补上了缺失的**门禁**——L0 技能/预检门禁、candidate 裁决、方法裁决、执行门禁（只有 Turing 跑代码，且只在门禁通过后）。7→10 人格，9→15 状态。
...
**路径 A — 执行层隔离（只有 Turing）**

Turing 要跑代码，不能纯上下文隔离。控制器调用 `prepare-turing-workspace`，用 `shutil.copy2` 把白名单文件复制到同盘临时目录，Turing 在这个 workspace 里执行 R/Python 脚本。结果被收集打包成 L7 delta JSON。

**我们不假装 `spawn_agent` 是操作系统沙箱**。路径 B 的隔离靠的是"信息不可见"——角色的上下文里根本没有不该看的内容，不是靠文件系统权限。真正的文件系统沙箱只有 Turing 的 workspace（路径 A）。
...
L7/L8 出结果后，L8.5（Curie 的第二实例）基于**实际结果的关键词**去检索 PubMed/EuropePMC，验证结论是否与已发表文献一致。找到的论文加入可增长的文献数据库（`09_Literature_Database/`），跨轮复用，通过 Obsidian wikilink 引用。
...
| 节点 | 角色 | 职责 | 隔离方式 |
|------|------|------|----------|
| L0 | Linnaeus | 预检、扫描技能、验证输入数据 | 路径 B |
| L1 | Einstein | 提出科学假设 | 路径 B |
| L2 | Feynman | 盲审攻击 L1 假设，找混淆因素 | 路径 B |
| L3 | Oppenheimer | 裁决假设，选出可测试的 | 路径 B |
| L4 | Fisher | 设计实验/分析策略 | 路径 B |
| L5 | Tukey | 从 EDA/QC 角度审查方法 | 路径 B |
| L6 | Oppenheimer | 批准或驳回分析方案 | 路径 B |
| L7 | Turing | 在受控 workspace 中执行脚本 | **路径 A** |
| L8 | Curie | 审计结果，验证可重复性 | 路径 B |
| L8.5 | Curie | 文献验证，增长文献数据库 | 路径 B |
| L9a | Feynman | 硬证伪（统计/逻辑完备性） | 路径 B（与 L9b 并行，互不可见） |
| L9b | Darwin | 生物学解读 | 路径 B（与 L9a 并行，互不可见） |
| L10a | Jobs | 价值评估，规划论文方向 | 路径 B |
| L10b | Oppenheimer | 终审：KEEP / REVISE / DOWNGRADE / DROP | 路径 B |
| L10c | Linnaeus | 聚合所有 delta，生成最终报告 | 读取所有 delta |
