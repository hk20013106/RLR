#!/usr/bin/env python3
"""Research Loop Room v0.3 — DAG-driven subagent architecture.

v0.3 converts each persona into an independent subagent with physical context
...
v0.2 is preserved untouched. v0.3 is a new standalone script.

Usage:
    python research_loop_v03.py --help
    python research_loop_v03.py demo
    python research_loop_v03.py new-project NAME [TOPIC]
    python research_loop_v03.py new-candidate PROJECT_DIR --title T --question Q --claim C --input I
    python research_loop_v03.py preflight PROJECT_DIR
    python research_loop_v03.py next-step PROJECT_DIR CAND_ID
    python research_loop_v03.py assemble-context PROJECT_DIR CAND_ID --node NODE
    python research_loop_v03.py emit-delta PROJECT_DIR CAND_ID --node NODE --persona P --file F
    python research_loop_v03.py decision PROJECT_DIR CAND_ID --status S --reason R [--route P]
    python research_loop_v03.py route PROJECT_DIR CAND_ID --to PERSONA --reason R
    ...
    python research_loop_v03.py show PROJECT_DIR CAND_ID
"""
...
def _mkdirs(project_dir):
    """v0.3 directory layout (same structure as v0.2)."""
    p = Path(project_dir)
...
# {name} - Research Loop Room v0.3 Index
...
def cmd_new_project(args):
    ...
    print(f"Created v0.3 project: {project_dir.resolve()}")
...
def cmd_demo(args):
    ...
    (pd / "00_Project_Index.md").write_text(
        _index_template_v03(name, "RLR v0.3 DAG demo"), encoding="utf-8")
    ...
    print(f"\nDemo v0.3 project created at: {pd.resolve()}")
    ...
    print(f"\n  python research_loop_v03.py list {pd}")
    print(f"  python research_loop_v03.py show {pd} {c1}")
    print(f"  python research_loop_v03.py aggregate-report {pd} {c1}")
    return 0
...
def build_parser():
    p = argparse.ArgumentParser(
        prog="research_loop_v03.py",
        description="Research Loop Room v0.3 - DAG-driven subagent architecture "
                    "(14 nodes, 10 personas, physical context isolation).")
    p.add_argument("--version", action="version", version=f"v{__version__}")
    sub = p.add_subparsers(dest="cmd", required=True)

    # demo
    sp = sub.add_parser("demo", help="generate a v0.3 demo project with DAG structure")
    sp.set_defaults(func=cmd_demo)

    # new-project
    sp = sub.add_parser("new-project", help="create a new v0.3 project folder")
