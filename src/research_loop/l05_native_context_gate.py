...
The historical ContextAssembler owns the legacy pre-research compatibility gate.
For native v2.1 L1, this wrapper validates the active frozen Curie binding first.
The canonical context assembler now selects compatibility pre-research from the
profile topology, so this gate no longer mutates the legacy global map.
"""
