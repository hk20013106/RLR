...
English ResearchSeed text.  Non-English semantic translation/planning belongs
to the configured provider boundary and is passed back through the existing
explicit-query path.  This module never selects records, retrieves source
bytes, verifies evidence, or creates an EvidencePack.
"""
...
_ROLES = {
    ...
    "shrew": ("shrews", "organism"),
    "shrews": ("shrews", "organism"),
    "mammal": ("mammals", "organism"),
...
def requires_provider_planning(seed: dict) -> bool:
    """Return whether preserving the ResearchSeed semantics requires a provider."""
    ...
    return contains_cjk(question) or contains_cjk(hypothesis)
...
def _concepts(seed: dict) -> list[dict]:
    ...
    if contains_cjk(question) or contains_cjk(hypothesis):
        raise CurieContractError(
            "non-English ResearchSeed requires provider-planned English retrieval queries"
        )
    ...
    for token in _TOKEN.findall(source):
        ...
        if normalized in _STOPWORDS:
            continue
        term, role = _ROLES.get(
            normalized,
            (
                normalized,
                "mechanism"
                if normalized.endswith(("ing", "tion", "ity"))
                else "phenomenon",
            ),
        )
        add(term, role, "seed")
...
def build_scientific_query_plan(
    ...
    by_role = lambda role: [
        item["term"] for item in concepts if item["role"] == role
    ]
    core = _unique(
        by_role("phenomenon")
        + by_role("organism")
    ...
    if reformulation_index:
        templates = [
            (
                "broad_biological_context",
                by_role("organism") + by_role("system") + by_role("phenomenon"),
            ),
            (
                "mechanism_axis",
                by_role("system") + by_role("mechanism")[:3] + ["primary evidence"],
            ),
            (
                "outcome_or_adaptation",
                by_role("outcome") + by_role("mechanism")[-3:] + ["adaptation"],
            ),
        ]
    else:
        templates = [
            ...
            queries.append(
                {
                    "intent": intent,
                    "query": rendered,
                    "concepts": _unique(terms or core),
                }
            )
    for suffix in (
