...
from .query_language import classify_supported_input_language, validate_english_retrieval_query
...
_ROLES = {"heart":("heart","system"),"cardiac":("cardiac","system"),"calcium":("calcium handling","mechanism"),"ca2+":("calcium handling","mechanism"),"adrenergic":("adrenergic regulation","mechanism"),"remodeling":("remodeling","outcome"),"injury":("injury","outcome"),"adaptation":("adaptation","outcome"),"mechanism":("mechanism","mechanism"),"shrew":("shrews","organism"),"shrews":("shrews","organism"),"mammal":("mammals","organism"),"mammals":("mammals","organism")}
...
def requires_provider_planning(seed: dict) -> bool:
    ...
    languages = {
        classify_supported_input_language(question, name="ResearchSeed scientific_question"),
        classify_supported_input_language(hypothesis, name="ResearchSeed hypothesis_seed"),
    }
...
def _concepts(seed: dict) -> list[dict]:
    question = _text(seed.get("scientific_question"), "ResearchSeed scientific_question")
    hypothesis = _text(seed.get("hypothesis_seed"), "ResearchSeed hypothesis_seed")
    for label, value in (("ResearchSeed scientific_question", question), ("ResearchSeed hypothesis_seed", hypothesis)):
        if classify_supported_input_language(value, name=label) != "en":
            raise CurieContractError("Chinese ResearchSeed requires provider-planned English retrieval queries")
...
def _bounded(values: list[str]) -> str:
    ...
    return validate_english_retrieval_query(" ".join(terms), name="English retrieval query")
...
def validate_scientific_query_plan(plan: dict) -> dict:
    ...
    for query in queries:
        ...
        rendered = validate_english_retrieval_query(query["query"], name="English retrieval query")
...
def build_scientific_query_plan(seed: dict, *, reformulation_index: int = 0) -> dict:
    ...
    if reformulation_index:
        templates = [("broad_biological_context", by_role("organism") + by_role("system") + by_role("phenomenon")), ("mechanism_axis", by_role("system") + by_role("mechanism")[:3] + ["primary evidence"]), ("outcome_or_adaptation", by_role("outcome") + by_role("mechanism")[-3:] + ["adaptation"])]
    else:
        templates = [("broad_biological_context", core[:5]), ("mechanism_axis", by_role("system") + by_role("phenomenon") + by_role("mechanism")[:4]), ("outcome_or_adaptation", by_role("phenomenon") + by_role("outcome") + by_role("mechanism")[-3:])]
    queries = []
    ...
    plan = {"schema_version":SCIENTIFIC_QUERY_PLAN_SCHEMA_VERSION,"planner":SCIENTIFIC_QUERY_PLANNER_VERSION,"language":"en","reformulation_index":reformulation_index,"concepts":concepts,"queries":queries[:MAX_QUERY_CANDIDATES]}
