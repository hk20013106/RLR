...
def cmd_aggregate_report(args):
    """L10c Linnaeus: report, required projection, then freeze round evidence."""
    project_dir = Path(args.project_dir)
    cf = _candidate_file(project_dir, args.cand_id)
    if not cf.exists():
        print(f"ERROR: no candidate {args.cand_id}", file=sys.stderr)
    ...
    # Shared reports remain human-readable pointers; candidate-scoped reports
    # above are the immutable round evidence candidates.
    ...
    # Obsidian is a REQUIRED current consumer, so successful L10c finalization
    # includes its projection. Do not freeze a round manifest for a round whose
    # required projection failed.
    sync_rc = cmd_obsidian_sync(
        SimpleNamespace(project_dir=str(project_dir), vault=getattr(args, "vault", None))
    )
    if sync_rc != 0:
        print("ERROR: L10c required Obsidian sync failed; round manifest not frozen.",
              file=sys.stderr)
        return 2

    # L10c is the sole owner of round-manifest creation. Large files remain in
    # place and are bound by exact path + SHA-256 only after finalization passes.
    try:
