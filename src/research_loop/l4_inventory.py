...
def _structured_model_invocation(
    ...
    try:
        ...
        if "execution_kind" not in message and "schema_path" not in message:
            raise
        command, prompt = dr.build_invocation(spec, node, question, claim, work_dir)
    ...
    if any(value in lowered for value in (
        "academic-research", "--plugin-dir", "--skill-path", "/ars-"
    )):
        raise dr.DeepResearchError(
    ...
    *,
    ...
    if known_sources is not None:
        command, _ = _structured_model_invocation(
            dr, spec, "L4", question, claim, work, inventory_schema_path
        )
    else:
    ...
    receipt = dr.skill_receipt(
        ...
        model=spec.model,
        **({"execution_kind": "structured_model"} if known_sources is not None else {}),
    )
    if known_sources is not None:
