...
classified failures into canonical maintenance events, invokes the existing
``meta_rlr.py run-once`` entry point, and resolves verified repairs through the
existing GitWorkspace provenance authority.
"""
...
def wake_meta_rlr_for_event(
    *,
    ...
    """Route one canonical event through the sole Meta-RLR repair authority."""
...
def maybe_wake_first_mile_failure(
    *,
    ...
    """Wake the existing owner for a repairable pre-L0 failure only."""
...
def maybe_wake_meta_rlr(
    *,
    ...
    """Run one existing Meta-RLR turn for a classified detached-worker failure."""
    environment = os.environ if environ is None else environ
    if (
        environment.get(AUTOWAKE_RETRY_GUARD_ENV)
        or not environment.get(AUTOWAKE_CONFIG_ENV)
        or not _provider_failure_is_repairable(status)
    ):
        return None
    try:
        repo_root = _repo_root()
